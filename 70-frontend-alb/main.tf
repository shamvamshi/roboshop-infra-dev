module "frontend_alb" {
  ...
  version = "9.16.0"
  internal = false # this is public load balancer
...
resource "aws_lb_listener" "frontend_alb" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "<h1>Hello iam from frontend ALB using https (443) </h1>"
      status_code  = "200"
    }
  }
}
...
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name    = "${var.environment}.${var.zone_name}" # dev.shamrobos.site
  type    = "A"
